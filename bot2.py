...
def fetch_news():
    ...
    try:
        ...
        if res.status_code != 200:
            return [], f"NewsAPI 오류: HTTP {res.status_code}"
        data = res.json()
...
"""
    ...
    try:
        ...
        if not text:
            return "- 한국어 요약 결과가 비어 있습니다.", "Gemini 응답은 왔지만 text가 비어 있음"
        return text, None
    except Exception as e:
        return "- 한국어 요약 생성에 실패했습니다.", f"Gemini 오류: {str(e)}"
...
def main():
    ...
    summary, summary_error = summarize_with_gemini(articles)
